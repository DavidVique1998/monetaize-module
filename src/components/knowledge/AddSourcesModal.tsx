...
export function AddSourcesModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/knowledge-bases/${knowledgeBase.knowledge_base_id}/sources`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (data.success) {
        ...
        setError(data.error || 'Error al agregar fuentes');
      }
    } catch (error: any) {
      console.error('Error adding sources:', error);
      setError('Error al agregar fuentes: ' + (error.message || 'Unknown error'));
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-2xl bg-white shadow-2xl flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Agregar Fuentes</h2>
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex gap-3">
          ...
          >
            {isLoading ? 'Agregando...' : 'Agregar Fuentes'}
          </button>
        </div>
      </div>
    </div>
